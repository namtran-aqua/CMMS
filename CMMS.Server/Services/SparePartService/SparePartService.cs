        ...
        public async Task<SparePartDto> CreateAsync(SparePartDto dto, UserDto currentUser)
        {
            ...
            const string sqlInsert = @"
                INSERT INTO dbo.Tbl_SparePart
                    (PartCode, PartName, CategoryID, Unit, Price, Inventory, MinStock, MaxStock, LocID, DeptID, SupplierID, Note, CreateDate, UpdateDate, CreateBy, IsCoded, ImageUrl, PartModel, FACID, SparePartBarcode)
                VALUES
                    (@PartCode, @PartName, @CategoryID, @Unit, @Price, @Inventory, @MinStock, @MaxStock, @LocID, @DeptID, @SupplierID, @Note, @CreateDate, @UpdateDate, @CreateBy, @IsCoded, @ImageUrl, @PartModel, @FACID, @SparePartBarcode);
